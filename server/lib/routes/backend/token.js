function token(dependencies) {
  ...
  const compile = (script, source, destination, next) => {
    ...
    var child = spawn("powershell.exe", [
      `Import-Module ${
        dependencies.root
      }/lib/tokenization/Invoke-PSImage.ps1; Invoke-PSImage`,
      `-Script ${dependencies.root}/lib/tokenization/${script}.ps1 -Image ${
        dependencies.root
      }/lib/tokenization/images/${source}.jpg -Out ${
        dependencies.root
      }/lib/tokenization/images/output/${destination}.png -Web`
    ]);
